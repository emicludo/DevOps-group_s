  ...
  try {
    ...
    if (req.body.follow) {
      ...
      if (!userFollowsList.includes(unfollowsUser.username)) {
        logger.log('warn',  { url: req.url ,method: req.method, requestBody: req.body , responseStatus: 404, message: "User is not following the user with name " + unfollowsUser.username });
        res.status(404).send("");
        return
      }
